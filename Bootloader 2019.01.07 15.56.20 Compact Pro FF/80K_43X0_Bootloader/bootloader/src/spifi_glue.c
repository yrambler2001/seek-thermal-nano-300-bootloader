/* spifi_glue.c — 80K_43X0_Bootloader (reconstructed)
 *
...
/* Shared device handle (IDA: dword_1001DCAC). flash_if.c also reads it.
 * Could equally be declared in bootloader.h; kept here to keep that header free
 * of lpcspifilib types. */
void *g_spifi_handle = NULL;

static volatile int      s_init_refcount = 0;   /* IDA: dword_1001DCB0[0] */
/* SPIFI lock refcount (IDA: dword_1001DD08[0]). In the image this cell shares a
 * base with lpcspifilib's family registry (whose one-shot guard is the [26]
 * word of the same block) — a linker placement detail; logically it is just
 * this lock. */
static volatile uint32_t s_spifi_lock    = 0;
/* Backing store for the device handle/context (IDA: buffer @0x1001DCB4). The
