/* startup_lpc43xx.c — 80K_43X0_Bootloader / Compact Android CW (reconstructed)
 *
 * Reset/CRT0 + vector table. FLASH-resident bootstrap: the part executes it
 * in place from 0x14000000, and its final act is to relocate the rest of the
 * image into SRAM and branch there.
 *
 * Every peripheral-interrupt vector points at a single trap, IRQ52_Handler.
 ...
 * The 0 entries in the IRQ range, plus the system-reserved slots, are NXP's
 * reserved vector positions. This build installs NO live interrupt handlers
 * of its own — there is nothing RAM-resident in the vector table beyond the
 * trap, and no interrupt callback/counter cells exist.
 */
...
/* Catch-all for every peripheral IRQ: a plain spin, NO wfi ("B ." only). */
...
void (* const g_pfnVectors[])(void) __attribute__((section(".vectors"), used)) =
{
    ...
    PendSV_Handler,            /* 14                                     */
    SysTick_Handler,           /* 15                                     */
    /* -------- external interrupts IRQ0..IRQ51 (LPC43xx M4) --------     */
    /* Every line is the trap; there are no live handlers in this build.  */
    ...
    IRQ52_Handler, IRQ52_Handler, IRQ52_Handler,                /* 40..42*/
    0,                                                          /* 43  r */
    IRQ52_Handler, IRQ52_Handler, IRQ52_Handler,                /* 44..46*/
    0,                                                          /* 47  r */
    IRQ52_Handler, IRQ52_Handler, IRQ52_Handler, IRQ52_Handler, /* 48..51*/
};
...
void Reset_Handler(void)
{
    ...
    *(volatile uint32_t *)STACK_GUARD_ADDR = STACK_GUARD_VALUE;  /* 0x1001F800 */
     ...
     * copy of the program does not exist yet, so we must not call into it.
       ...
       Observed addresses in this image:
         zero BSS   : 0x15C bytes at 0x1001DA40   (handle/handle-ptr/board cells)
         empty pass : 0x1001DB9C..0x1001DB9C
         copy .data : 0x440 bytes flash 0x140019DC -> 0x1001D600  (config + keys)
         copy code  : 0x1600 bytes flash 0x140003B8 -> 0x1001C000 */
 ...
 * (0x1001C1B4). Reset_Handler has just copied .text_ram there, so this call
 * transfers into SRAM. The listing does the equivalent literal BX 0x1001C1B5. */
