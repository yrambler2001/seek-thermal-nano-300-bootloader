/* crypto_stream.c — 80K_43X0_Bootloader / Compact Android CW (reconstructed)
 *
 ...
 *
 * In this build these routines are compiled at -O0, so the listing carries an
 * R7 frame pointer and spills every local to the stack; the C below is the
 ...
 *  - When the source lies in the SPI-NOR window (top byte 0x14), each <=16-word
 *    chunk is staged into a RAM scratch buffer via mem_read before processing;
 *    a RAM source is consumed in place. This is an I/O detail; the result is
 *    identical. (mem_read here is a command-mode SPIFI read — see flash_if.c.)
 */
...
/* Seed from the embedded Key A (flash 0x140019E0 / RAM 0x1001D604). */
 ...
 * bits are zero too. (Key A yields 0x0000FFFF here; Key B yields 0xFA3BEA91.) */
