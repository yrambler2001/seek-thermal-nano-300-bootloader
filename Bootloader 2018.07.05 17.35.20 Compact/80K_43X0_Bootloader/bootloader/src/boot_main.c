/* boot_main.c — 80K_43X0_Bootloader / Compact Android CW (reconstructed)
 *
 * Top-level boot stage (executed from SRAM) plus slot selection, the
 * key-agnostic acceptance test, and the word-zeroing helper. Names are kept
 ...
 *
 * Right after the SPIFI controller is brought up, this build also runs a board
 * GPIO/timer bring-up step (board_gpio_init_timer_delay) before touching the
 * slots. The flash bus is left in command mode for the whole sequence and is
 * still in command mode when the application is entered.
 */
 ...
 * requires magic + length < 0x10000, then tries Key B/device, then Key A,
 * accepting the first whose full 32-bit plaintext checksum is exactly
 * 0x0000FFFF. Returns 1 (Key B/device), 2 (Key A), 0 (none).
 *
 * The decision is the checksum alone: a key that checksums correctly is
 * accepted outright, and the header is read with the plain mem_read path. */
int image_try_keys(const uint32_t *slot_base)
{
    ...
    mem_read(hdr, slot_base + 128, 0x40u);
...
uint32_t select_boot_slot(int update_flag)
{
    ...
    if (update_flag) {
        ...
        if (cur == SLOT_A_BASE || cur == SLOT_B_BASE) {
            ...
            flash_program(cur + IMG_HEADER_OFFSET, &update_flag, 4u);
            cfg[0] = 2;
            if (flash_erase_block(BOOT_CONFIG_BASE) == FL_OK)
                flash_program(BOOT_CONFIG_BASE, cfg, 0x10u);
        }
...
void boot_main(void)
{
    ...
    spifi_init();                                      /* controller up, command mode */
    board_gpio_init_timer_delay();                     /* board GPIO + ~200 ms settle */
    ...
    /* publish handoff block, jump the entry */
    mem_read((void *)RAM_APP_LOAD_BASE, segtab, 512);
    *(volatile uint32_t *)(RAM_HANDOFF_INFO + 0x00) = BUILD_INFO_ADDR;        /* info ptr 0x140019B8 */
